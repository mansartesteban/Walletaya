<template>
  <div ref="menu" class="menu" :class="{ 'full-width': fullWidth }">
    <template v-if="!noActivator">
      <slot name="activator" v-bind="{ on: activatorEvents }">
        <Btn class="menu-activator" icon="chevron-down" @click="activatorEvents.onClick">
          <slot name="activator-label" v-bind="{ option: model }"></slot>
        </Btn>
      </slot>
    </template>
    <div ref="menuPanel" class="menu-panel" :class="{ opened }" :style="fullWidth
        ? 'z-index: 1000; bottom: 0; left: 0; right: 0; width: unset'
        : ''
      ">
      <InputText v-if="search" v-model="searchText" @focus="focus" @blur="unfocus" label="Recherche un token"
        class="search m-sm" />
      <RecycleScroller :items="filteredOptions" :item-size="48" key-field="value" v-slot="{ item: option }"
        class="blurry-container overflow-x-hidden">
        <slot name="option" v-bind="{ on: optionEvents, option: option }">
          <div @click="optionEvents.onClick(option)" class="option flex gap-md"
