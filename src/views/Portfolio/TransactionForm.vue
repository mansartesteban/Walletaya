<template>
  <div class="transaction-form flex flex-column gap-md">
    <div class="form-line">
      <ButtonGroup v-model="transactionType" :items="transactionTypes"></ButtonGroup>
    </div>
    ...
    <Card noHeader v-if="[1, 2].includes(transactionType)">
      <div class="flex flex-column gap-md">
        Débit
        <div class="form-line flex gap-md" :class="{ 'flex-reverse': settings.leftHanded }">
          <InputNumber v-model="debitAmount" label="Montant" :append="debitToken?.symbol">
          </InputNumber>
          <Btn v-if="transactionType === 1" icon="magic" @click="debitAutoCalculate"></Btn>
          <TokenPicker v-model="debitToken" @changed="debitChangedToken"></TokenPicker>
        </div>
        <div class="form-line flex gap-md">
          <InputNumber v-model="debitValue" label="Valeur" append="USDT"></InputNumber>
        </div>
    ...
    <Btn v-if="transactionType === 1" icon="exchange" class="my-n-xl mx-auto" size="lg" rounded style="z-index: 1"
      @click="swap"></Btn>
    ...
    <Card noHeader v-if="[0, 1].includes(transactionType)">
      <div class="flex flex-column gap-md">
        Crédit
        <div class="form-line flex gap-md" :class="{ 'flex-reverse': settings.leftHanded }">
          <InputNumber v-model="creditAmount" label="Montant" :append="creditToken?.symbol">
          </InputNumber>
          <Btn v-if="transactionType === 1" icon="magic" @click="creditAutoCalculate"></Btn>
          <TokenPicker v-model="creditToken" @changed="creditChangedToken"></TokenPicker>
        </div>
        <div class="form-line flex gap-md">
          <InputNumber v-model="creditValue" label="Valeur" append="USDT"></InputNumber>
        </div>
    ...
    <div class="flex gap-md" :class="{ 'flex-reverse': settings.leftHanded }">
      <Btn v-if="transactionId" @click="deleteTransaction(transactionId)" icon="delete" icon-reverse severity="error"
        elevated>
        Supprimer
      </Btn>
      <Btn @click="saveTransaction(transactionId)" icon="save" icon-reverse elevated severity="success">
...
const creditChangedToken = (v) => {
  creditValue.value = transactionId.value ? creditValue.value : tokenListStore.prices[v.id]
}
const debitChangedToken = (v) => {
  debitValue.value = transactionId.value ? debitValue.value : tokenListStore.prices[v.id]
}
